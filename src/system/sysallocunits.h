...
struct sysallocunits_row
{
    //FIXME:  to be tested
    struct data_type {
        uint64          auid;           // auid(allocation_unit_id / partition_id) - 8 bytes - the unique ID / primary key for this allocation unit.
                                        // The top 16 bits of this ID is 0 - 4, 255 or 256.  
                                        // The mid 32 bits is the Allocation Unit's ObjectID (which is an auto-increment number), 
                                        // and the lowest 16 bits appear to always be 0.
