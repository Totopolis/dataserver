...
struct bootpage_row
{
    ...
    struct data_type { // IS_LITTLE_ENDIAN
        ...
        char        dbi_modDate[8];                 // 0x2A4 : dbi_modDate - 8 bytes
        char        dbi_verPriv[4];                 // 0x2AC : dbi_verPriv - 4 bytes
        ...
        char        dbi_AuIdNext[8];                // 0x2E0 : dbi_AuIdNext - 8 bytes
...
struct bootpage_row_meta {
    ...
    typedef_col_type(bootpage_row, dbi_createVersion);
    typedef_col_type(bootpage_row, dbi_status);
    typedef_col_type(bootpage_row, dbi_nextid);
    typedef_col_type(bootpage_row, dbi_crdate);
    typedef_col_type(bootpage_row, dbi_dbname);
    typedef_col_type(bootpage_row, dbi_dbid);
    typedef_col_type(bootpage_row, dbi_maxDbTimestamp);
    typedef_col_type(bootpage_row, dbi_checkptLSN);
    typedef_col_type(bootpage_row, dbi_differentialBaseLSN);
    typedef_col_type(bootpage_row, dbi_dbccFlags);
    typedef_col_type(bootpage_row, dbi_collation);
    typedef_col_type(bootpage_row, dbi_familyGuid);
    typedef_col_type(bootpage_row, dbi_maxLogSpaceUsed);
    typedef_col_type(bootpage_row, dbi_recoveryForkNameStack);
    typedef_col_type(bootpage_row, dbi_differentialBaseGuid);
    typedef_col_type(bootpage_row, dbi_firstSysIndexes);
    typedef_col_type(bootpage_row, dbi_createVersion2);
    typedef_col_type(bootpage_row, dbi_versionChangeLSN);
    typedef_col_type(bootpage_row, dbi_LogBackupChainOrigin);
    typedef_col_type(bootpage_row, dbi_modDate);
    typedef_col_type(bootpage_row, dbi_verPriv);
    typedef_col_type(bootpage_row, dbi_svcBrokerGUID);
    typedef_col_type(bootpage_row, dbi_AuIdNext);
