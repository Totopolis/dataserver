...
#pragma pack(push, 1) 
	...
	struct page_header // 96 bytes page header
	{
		...
		bool is_null() const {
			return data.type == pageType::null;
		}
	};
...
#pragma pack(pop)
	...
	inline T const * page_body(page_header const * p) {
		if (p) {
			...
			A_STATIC_ASSERT(sizeof(T) <= page_header::body_size);
...
#define typedef_col_type(pagetype, member) \
	...
	struct page_info {
		...
		static std::string type(page_header const &, pageId_t);
		static std::string type_raw(char const * buf, size_t buf_size, bool show_address);

		template<class T>
		static std::string type_raw_t(T & buf, bool show_address) {
			return type_raw(buf, A_ARRAY_SIZE(buf), show_address);
		}
		template<class T>
		static std::string type_pod(T & buf) {
			A_STATIC_ASSERT_IS_POD(T);
			return type_raw((char const *)(&buf), sizeof(buf), false);
		}
