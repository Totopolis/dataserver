...
//
#pragma once
#ifndef __SDL_UTILS_UTF_HPP__
#define __SDL_UTILS_UTF_HPP__
...
namespace utf {
    ...
    #define SDL_LOCALE_LIKELY(x)   (x)
    #define SDL_LOCALE_UNLIKELY(x) (x)
    ...
    struct utf_traits<CharType,1> {
        ...
        static int trail_length(char_type ci) 
        {
            ...
            if(c < 128)
                return 0;
            if(SDL_LOCALE_UNLIKELY(c < 194))
                return -1;
            if(c < 224)
                return 1;
            if(c < 240)
                return 2;
            if(SDL_LOCALE_LIKELY(c <=244))
                return 3;
            return -1;
        }
        ...
        static int width(code_point value)
        {
            ...
            else if(SDL_LOCALE_LIKELY(value <=0xFFFF)) {
                return 3;
            }
            else {
                return 4;
        ...
        static code_point decode(Iterator &p,Iterator e)
        {
            ...
            if(SDL_LOCALE_UNLIKELY(trail_size < 0))
                return illegal;
            ...
            // valid range
            if(SDL_LOCALE_UNLIKELY(!is_valid_codepoint(c)))
                return illegal;

            // make sure it is the most compact representation
            if(SDL_LOCALE_UNLIKELY(width(c)!=trail_size + 1))
                return illegal;
        ...
        static code_point decode_valid(Iterator &p)
        {
            ...
            if(lead < 224)
                trail_size = 1;
            else if(SDL_LOCALE_LIKELY(lead < 240)) // non-BMP rare
                trail_size = 2;
            else
                trail_size = 3;
    ...
    struct utf_traits<CharType,2> {
        ...
        {
            if(SDL_LOCALE_UNLIKELY(current == last))
                return incomplete;
            uint16_t w1=*current++;
            if(SDL_LOCALE_LIKELY(w1 < 0xD800 || 0xDFFF < w1)) {
                return w1;
            }
            if(w1 > 0xDBFF)
                return illegal;
    ...
    struct utf_traits<CharType,4> {
        ...
        {
            if(SDL_LOCALE_UNLIKELY(current == last))
                return sdl::locale::utf::incomplete;
            code_point c=*current++;
            if(SDL_LOCALE_UNLIKELY(!is_valid_codepoint(c)))
                return sdl::locale::utf::illegal;
            return c;
        }
...
#endif // #ifndef __SDL_UTILS_UTF_HPP__
